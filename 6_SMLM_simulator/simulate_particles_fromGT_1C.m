...
subplot(1,2,2)
scatter(sim_cent{ID, 1}(:,1),sim_cent{ID, 1}(:,2),10,'filled');
axis([-300 300 -300 300])
% axis equal
axis square
box on
...
close all
figure('Position',[300 100 1000 300]);
...
subplot(1,3,2)
scatter3(simCent_wNoise{ID, 1}(:,1),simCent_wNoise{ID, 1}(:,2),simCent_wNoise{ID, 1}(:,3),10,'filled');
axis([-300 300 -300 300]);
view([90,90]);
title(['simulation LE = ' num2str(labelling_eff)]);
% axis equal
box on
axis square
...
heigth = round((max(simCent_wNoise{ID, 1}(:,1)) - min((simCent_wNoise{ID, 1}(:,1))))/pxlsize);
width  = round((max(simCent_wNoise{ID, 1}(:,2)) - min((simCent_wNoise{ID, 1}(:,2))))/pxlsize);
